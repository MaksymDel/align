...
def convert_examples_to_features(examples, seq_length, tokenizer, lang):
    """Loads a data file into a list of `InputFeature`s."""

    features = []
    for (ex_index, example) in enumerate(examples):
        tokens_a = tokenizer.tokenize(example.text_a, lang=lang)

        tokens_b = None
        if example.text_b:
            tokens_b = tokenizer.tokenize(example.text_b, lang=lang)
...
def main():
    ...
    features = convert_examples_to_features(
        examples=examples, seq_length=args.max_seq_length, tokenizer=tokenizer, lang=lang)
    ...
    for input_ids, input_mask, example_indices in tqdm(eval_dataloader):
            ...
            all_encoder_layers = model(input_ids)[1]
