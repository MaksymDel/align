...
class XnliBert(Model):
    ...
    def __init__(self, 
        ...
        super(XnliBert, self).__init__(vocab, regularizer)
        ...
        in_features = self.bert_model.config.hidden_size * 2
    ...
    def forward(self,  # type: ignore
        ...
        type_ids_premise = premise[f"{self._index}-type-ids"]
        type_ids_hypothesis = hypothesis[f"{self._index}-type-ids"]
        
        mask_premise = (ids_premise != 0).long()
        mask_hypothesis = (ids_hypothesis != 0).long()

        _, pooled_premise = self.bert_model(input_ids=ids_premise,
                                    token_type_ids=type_ids_premise,
                                    attention_mask=mask_premise)

        _, pooled_hypothesis = self.bert_model(input_ids=ids_hypothesis,
                            token_type_ids=type_ids_hypothesis,
                            attention_mask=mask_hypothesis)
        ...
        logits = self._classification_layer(concatenated_input)
